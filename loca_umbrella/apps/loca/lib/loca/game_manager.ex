defmodule Loca.GameManager do
    ...
    def init(list) do
        {:ok, %{markers: list, player: %{}}}
    end

    def get_state(game_id) do
        GenServer.call({:global, game_id}, :check_state)
    end
    ...
    defp update_state(state, position, :on_point) do
        %{state | markers: tl(state.markers), player: %{state.player | position: position}}
    end
    defp update_state(state, _position, :no_movement), do: state
    defp update_state(state, position, _result) do
